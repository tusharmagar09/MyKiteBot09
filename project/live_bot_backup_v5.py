...
def run(dry_run=False):
    ...
    try:
        ...
        # Pre-rank RS for entire universe
        universe_rs = []
        for symbol, df in price_data.items():
            i = len(df) - 1
            rs_score = rs.compute_rs_score(df, rs_df, i)
            if rs_score is not None:
                universe_rs.append((symbol, rs_score, i, df))

        universe_rs.sort(key=lambda x: x[1], reverse=True)
        top_30_idx = max(1, int(len(universe_rs) * 0.30))
        top_candidates = universe_rs[:top_30_idx]

        # Check entry signals on top 30% only
        signals = []
        for symbol, rs_score, i, df in top_candidates:
        ...
        # Apply slot limits
        slots = config.MAX_POSITIONS - len(current_portfolio)
        candidates = signals[:max(0, slots)]
...
def shutdown_ec2(dry_run=False):
    ...
    if dry_run:
        logger.info("[DRY RUN] Would shutdown EC2 now.")
        return
